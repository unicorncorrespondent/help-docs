...
> **注意**：Binder 的总长度需满足 **3–250 个氨基酸**的限制。
>
> 如果链长度不在此范围内，需要：
> - 通过设置 motif 保留关键片段
> - 或重新选择其他满足要求的链
...
motif 连接区域的长度默认与参考 Binder 中保持一致，如需调整，您可以点击 **铅笔样式的编辑按钮**，为每个连接区域分别设置长度范围。系统会根据 **motif + motif 连接区域** 自动计算 Binder 的总长度范围。具体长度设置建议见[设计参数](./C-02-04-DesignParameters.md)


> ⚠️ 注意：最终 Binder 总长度必须满足 **3–250 个氨基酸**的限制。

上一步：[结构输入](./C-02-02-InputTarget.md)
下一步：[参考结构](./C-02-04-DesignParameters.md)
