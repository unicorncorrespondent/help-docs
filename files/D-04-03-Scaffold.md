# 定义骨架与生长位点

> 设置分子设计的起始骨架和生长位点

完成结合位点定义后，需要指定分子设计的起始骨架和生长位点。骨架是分子设计的起点，生长位点是新片段将要连接的位置。HelixVS-Syn提供两种方式输入骨架与生长位点：
- 直接在文本框中输入骨架的 SMILES 字符串
- 使用内置的分子绘制工具绘制骨架结构
<img src="https://bml-test-test.bj.bcebos.com/helix_upload/%E9%AA%A8%E6%9E%B6%E5%88%86%E5%AD%90%E7%94%9F%E6%88%90%20%E9%AA%A8%E6%9E%B6%20-80e981a1.jpg" style="zoom:30%;" />

输入或绘制完成后，系统会自动展示：

- **骨架结构图**：显示骨架的二维结构
- **节点编号**：骨架上的原子按编号排列

<img src="https://bml-test-test.bj.bcebos.com/helix_upload/%E9%AA%A8%E6%9E%B6%E5%88%86%E5%AD%90%E7%94%9F%E6%88%90%20%E9%AB%98%E4%BA%AE%20%E4%B8%AD%E6%96%87-e473eca6.jpg" style="zoom:50%;" />

您可以在下方『指定骨架上的片段生长位点』的输入框中输入编号（1个或多个）来指定骨架上的片段生长位点。完成输入后，选中的位点会高亮在骨架上。


骨架的结构和每个节点的编号将会自动展示在文本框下。

<img src="https://bml-test-test.bj.bcebos.com/helix_upload/%E9%AA%A8%E6%9E%B6%E5%88%86%E5%AD%90%E7%94%9F%E6%88%90%20%E9%AB%98%E4%BA%AE%20%E4%B8%AD%E6%96%87-e473eca6.jpg" style="zoom:50%;" />


---

上一步：[定义结合位点](./D-04-02-BindingSite.md)
下一步：[查看结果](./D-04-04-Results.md)

---

## 原始来源

本文档内容整理自 PaddleHelix 帮助文档的 HelixVS-Syn 骨架定义章节。以下是原始文档中的章节定位信息，供您追溯和参考：

| 章节标题 | 源文档锚点 ID | 行号范围 |
|----------|---------------|----------|
| 第三步：定义骨架和生长位点 | 4f684ec | 行 4368-4387 |

**源文档完整路径**：`../../../../../../../../02-Input/files/HelpDoc-20260126.md`
