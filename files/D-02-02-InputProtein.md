...
在进行虚拟筛选之前，首先需要准备好靶蛋白的三维结构。蛋白结构是分子对接的基础，其质量直接影响筛选结果的可靠性。HelixVS 支持从 RCSB PDB 数据库搜索已有结构，也可以上传本地保存的蛋白文件。本节将详细介绍这两种输入方式的操作步骤、文件格式要求以及蛋白预处理的相关知识，帮助您正确准备虚拟筛选所需的蛋白结构数据。
...
上一步：[HelixVS 概述](./D-02-01-HelixVS.md)
下一步：[定义结合位点](./D-02-03-DefineBindingSite.md)
